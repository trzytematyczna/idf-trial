...
model_dir <- paste0("./results/guardian-lda/articles/models/ngram_1:",ngram,"/alpha",al)
name<-paste("_ngram",ngram, "_al",formatC(al, width=2, flag = "0"), "_k",k_list, sep="")
res_dir <- paste0("./results/guardian-lda/articles/topics/")
##################


csv_data <- read.csv2("./data/guardian/full_articles_guardian.csv", stringsAsFactors = FALSE)
...
# set.seed(1234)
pdf(paste(res_dir,"cluster",name,".pdf",sep=""))
...
selected.articles %>% write.csv2(paste0(res_dir,"guardian_articles_selected_lda", name,".csv"))
