...
# rds_dir <- paste0("./results/",data_name,"/k-",k_list,"/")
model_dir <- paste0("./results/",data_name,"/")
model_dir_p <- paste0("./results/",data_name,"/")
...
g<-ggplot(topic.perc, aes(x=reorder(topic, perc),y=perc))+
  geom_bar(stat="identity",position="stack")+
  ylim(0.0, (max(topic.perc$perc)))+
  theme(axis.text.x = element_text(angle = 90))+
  ggtitle(paste0("Percentage of number of tweets in each topic / 2M sample"))
ggsave(paste0(res_dir,data_name,"-",exp_name,"-topic-perc.pdf"))



read.model.fun.p <- function(k){
  filename = file.path(model_dir_p, paste0(k, model_name))
  if (!file.exists(filename)) {
    print("Nofile!")
  } else {
    load(filename)
...
model_list_p <- TmParallelApply(X = p_list, FUN = read.model.fun.p)

model_p<-model_list_p[[1]]


document_topic_p <- data.frame(model_p$theta)
document_topic_p$document <-rownames(document_topic_p) 
rownames(document_topic_p) <- 1:nrow(document_topic_p)
document_topic_p <- document_topic_p %>% 
  reshape2::melt(id.vars = "document") %>% 
  rename(topic = variable) %>% 
  tidyr::separate(topic, into =c("t","topic")) %>% 
  select(-t) %>% 
...
# document_topic %>% str_replace("X","")
# if(data_name %like% "twitter"){
document_topic_p$document<-document_topic_p$document %>%stri_replace_all_fixed("X","")

topic.top1.word <- terms.summary %>% group_by(topic) %>% top_n(2)

topic.top1.word <- topic.top1.word %>% group_by(topic, word) %>% filter(row_number() == 1) %>%
  ungroup() %>% tidyr::separate(topic, into =c("t","topic")) %>% select(-t)

# topic.top1.word$labs <- topic.top1.word%>% 
#   group_by(topic) %>%
#   mutate(x=paste(topic))%>%
#   summarise(s=paste(x, paste(word, collapse = ","), collapse=".")) %>%
#   paste(s," (",topic,")",sep="")

topic.labels<-topic.top1.word%>% 
  group_by(topic) %>%
...
topic.labels.1<-topic.labels %>% rbind(c("6","asd, asd 6"))

dt_full<- merge(document_topic,document_topic_p, by="document")
...
topic.x.y <- dt_full%>% 
  ...
  mutate(perc=n/sum(n))%>%
  ungroup()%>%
  mutate(topic.x=factor(topic.x,levels = 1:5, labels=topic.labels$z))%>%
  mutate(topic.y=factor(topic.y,levels = 1:6, labels=topic.labels.1$z))


g<-ggplot(topic.x.y, aes(x=topic.y,y=perc))+
  geom_bar(stat="identity",position="stack")+
  ylim(0.0, (max(topic.x.y$perc)+0.05))+
  ...
  # geom_text(data = topic.labels, aes(topic,z))+
  ggtitle(paste0(data_name,"-",exp_name))+
  xlab(paste0("topics from k=",p_list))+
  ylab(paste0("percentage of topics from k=",p_list," included in topic from k=",k_list))+
  # scale_x_discrete(breaks = xbreaks)+
  facet_wrap(.~topic.x, ncol=2)#, labeller = topic.labels$z)
g

ggsave(paste0(res_dir,data_name,"-",exp_name,"-k-",k_list,"x-k-",p_list,".pdf"))
