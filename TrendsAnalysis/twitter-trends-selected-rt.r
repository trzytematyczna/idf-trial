...
grouped.sp.rt<-grouped.retweets.only%>%
...
#only segments
p <- ggplot(data.selected, aes(x=index, y=sum_probability)) +
  geom_step() +
  # geom_line()+
...
ggsave(paste0("./results/twitter-trained/trends-green-t",selected.topic,".pdf"), device = "pdf")
p
...
q <- ggplot(data.selected, aes(x=index, y=sum_probability)) +
  ...
  geom_rect(data=trends_trend.selected,
            inherit.aes = FALSE,
            aes(xmin=index, xmax=index+1, ymin=0, ymax=max(data.selected$sum_probability), fill=trend),
            alpha=0.3)

# facet_grid(topic~., scales = "free_y")
q
ggsave(paste0("./results/twitter-trained/trends-colors-t",selected.topic,".pdf"), device = "pdf")


library(gridExtra)

pdf("trends-joined-t",selected.topic,".pdf")
